...
describe('GraphQLDate', () => {
    it('should fail serializing', () => {
        ...
        return graphql(schema, query).then(result => {
            ...
            expect(errors[0].message).toEqual(`Date can't represent non-date value: ${value}`)
        })
    })
    ...
    `
        return graphql(schema, query).then(result => {
            const errors = result.errors
            expect(errors).toBeDefined()
            expect(errors[0].message).toEqual(`Expected date value but got: ${value}`)
        })
    })
    ...
    `
        return graphql(schema, query, null, null, { date: value }).then(result => {
            expect(result.errors).toBeUndefined()
        })
    })
