...
describe('GraphQLEmailAddress', () => {
    it('should fail serializing', () => {
        ...
        return graphql(schema, query).then(result => {
            ...
            expect(errors[0].message).toEqual(`EmailAddress can't represent value: ${value}`)
        })
    })
    ...
    `
        return graphql(schema, query).then(result => {
            const errors = result.errors
            expect(errors).toBeDefined()
            expect(errors[0].message).toEqual(`Expected email address value but got: ${value}`)
        })
    })
    ...
    `
        return graphql(schema, query, null, null, { emailAddress: value }).then(result => {
            expect(result.errors).toBeUndefined()
        })
    })
})
